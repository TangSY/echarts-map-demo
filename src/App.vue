  ...
  methods: {
    ...
    downloadNameAndCode () {
      ...
      function getData (data) {
        ...
        _this.$ba&&this.$ba.trackEvent('echartsMap','全国省市区县街道和编码（不包含边界数据）下载',filename);
        saveAs(blob,`${filename}.json`); //filename
      }
    },
    echartsMapClick (params) {//地图点击事件
      this.$ba&&this.$ba.trackEvent('echartsMap','点击地图',`${params.data.name}-${params.data.cityCode}`);
    ...
    downloadJson (nameType) {//geo文件下载
      ...
      if(nameType==='area') {
        this.$ba&&this.$ba.trackEvent('echartsMap','文件下载','下载级联数据');
        this.$refs.dialog.show();
        return;
      }
      if(nameType==='all') {
        this.$ba&&this.$ba.trackEvent('echartsMap','文件下载','打包下载全部');
        this.$refs.dialog.show();
        return;
      }
      if(nameType==='street') {
        this.$ba&&this.$ba.trackEvent('echartsMap','文件下载','下载乡镇数据');
    ...
    search (area) {
      ...
      this.$ba&&this.$ba.trackEvent('echartsMap','筛选地图',`${this.cityName}-${this.cityCode}`);
